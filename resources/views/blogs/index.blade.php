    ...
    <section style="padding-top: 150px; padding-bottom: 50px;">
        ...
        <div class="blog-listing blog-listing-classic no-margin-top wow fadeIn">
            ...
            <div class="blog-details">
                <div class="blog-date">Posted by <a href="{{ route('blogger.profile', $blog->user->id) }}"><b>{{ $blog->user->name }}</b></a> | {{ date('F d, Y', strtotime($blog->created_at)) }} | <a href="{{ route('blog.categorywise', str_replace(" ", "-", $category->name))) }}">{{ $blog->blogcategory->name }}</a> </div>
