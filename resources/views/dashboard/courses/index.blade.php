    ...
    <div class="container-fluid">
            <div class="row">
                <div class="col-md-12"> @php
                        ...
                        // কালেকশনকে ক্যাটাগরি অনুযায়ী গ্রুপ করা
                        ...
                        <div class="card card-outline card-primary mb-4 shadow-sm">
                            <div class="card-header bg-light">
                                <h3 class="card-title font-weight-bold text-primary">
                                    ...
                                    <span class="badge badge-pill badge-secondary ml-2">{{ $courseGroup->count() }} টি</span>
                                </h3>

                                @if($loop->first) {{-- শুধু প্রথম কার্ডে 'নতুন কোর্স যোগ' বাটন দেখাচ্ছি --}}
                            ...
                            <div class="card-body p-0">
                                <table class="table table-hover table-striped mb-0">
                                    <thead class="thead-light">
                                        <tr>
                                            <th width="30%">কোর্সের নাম</th>
                                            <th>স্ট্যাটাস</th>
                                            <th>ধরন</th>
                                            <th>পরীক্ষা সংখ্যা</th>
                                    ...
                                    @foreach($courseGroup as $course)
                                        <tr>
                                            <td>
                                                <a href="{{ route('dashboard.courses.add.exam', $course->id) }}" class="text-bold text-dark">
                                                    {{ $course->name }}
                                                </a>
                                            </td>
                                            <td>
                                                ...
                                                @else
                                                    <span class="badge badge-danger"><i class="fas fa-times-circle"></i> In-active</span>
                                                @endif
                                            ...
                                            <td>
                                                <small class="text-muted">
                                                    @if($course->type == 1) সাধারণ @elseif($course->type == 4) ফ্রি @elseif($course->type == 5) প্রশ্ন ব্যাংক @endif
                                                </small>
                                            </td>
                                            <td>
                                                <span class="badge badge-info">{{ $course->courseexams->count() }} টি পরীক্ষা</span>
                                            </td>
                                            <td>
                                                <div class="btn-group">
                                                    <a href="{{ route('dashboard.courses.add.exam', $course->id) }}" class="btn btn-default btn-sm" title="পরীক্ষা যোগ">
                                                        <i class="fas fa-folder-plus text-warning"></i>
                                                    </a>
                                                    <a href="{{ route('dashboard.courses.exam.serial.edit', $course->id) }}" class="btn btn-default btn-sm" title="সিরিয়াল">
                                                        <i class="fas fa-sort-amount-up-alt text-success"></i>
                                                    </a>
                                                    <button type="button" class="btn btn-default btn-sm" data-toggle="modal" data-target="#editCourseModal{{ $course->id }}" title="এডিট">
                                                        <i class="far fa-edit text-primary"></i>
                                                    </button>
                                                    <button type="button" class="btn btn-default btn-sm" data-toggle="modal" data-target="#changeExamDatesModal{{ $course->id }}" title="তারিখ পরিবর্তন">
                    ...
                    @if($courses->isEmpty())
                        <div class="alert alert-info text-center">কোনো কোর্স পাওয়া যায়নি।</div>
                    @endif
